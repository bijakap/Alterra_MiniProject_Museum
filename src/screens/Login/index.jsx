            ...
            <div className="pt-10">
                {
                  ...
                  :
                  <button type="submit" className="w-full flex justify-center bg-[#0B3B36] text-gray-100 p-3 rounded-lg tracking-wide font-semibold cursor-pointer" onClick={(e) => {handleLogin(e)}}>
                  Masuk
                  </button>
                }
                {
                  message.length > 1 ?
                  <p className="text-sm font-medium text-red-600 text-center pt-2">{message}</p>
                  :
                  <></>
                }
            </div>
        </form>
        <p className="text-sm pt-10">Belum memiliki akun? <Link to="/daftar"><span className="text-[#0B3B36] font-[600] cursor-pointer">Daftar</span></Link></p>
      </div>
  </div>
  )
}
