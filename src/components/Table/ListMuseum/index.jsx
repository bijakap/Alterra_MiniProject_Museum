...
const ListMuseum = ({data, loadingMessage, animate, DeleteMuseum}) => {
  ...
  return (
    <>
      {
        data.length <= 0 ? <span className={`flex justify-center text-3xl ${animate === true ? "animate-pulse" : "" }`}>{loadingMessage}</span> : 
        ...
        <tbody className=''>
          {data.map((data, index) => (
            <tr key={index} className={`text-left font-normal text-[14px] leading-[24px] tracking-[0.25px] text-[#232D42] ${index % 2 !== 0 ? "bg-[#F4FAFF]" : "bg-white"}`}>
              ...
              <td className='px-4 py-2 w-[180px] h-[130px]'><img src={data.gambar} className="w-full h-full object-cover rounded-xl"/></td>
              <td className='px-4 py-2'>{data.nama}</td>
              <td className='px-4 py-2'>{data.alamat}</td>
              <td className='px-4 py-2 w-[250px]'><p className='line-clamp-3'>{data.deksripsi}</p></td>
              <td className='text-center p-2'>{data.jumlahUlasan}</td>
              <td className='px-4 w-[100px]'>
                <button className='flex w-full my-2 px-6 py-2 rounded-lg border bg-white hover:bg-[#116E1C] text-[#116E1C] hover:text-white text-[14px] leading-[20px] tracking-[0.25px]' onClick={() => {navigate("/admin/edit/" + data.id)}}>
                    <EditIcon className='text-[20px]'/> <span className='self-center font-medium ml-2'>Edit</span>
                </button>
                <button className='flex w-full my-2 px-6 py-2 rounded-lg border bg-white hover:bg-red-700 text-red-700 hover:text-white text-[14px] leading-[20px] tracking-[0.25px]' onClick={() => {handleDelete(data.id)}}>
