  ...
  return(
    <div className="flex flex-col">
          ...
          return(
            ...
            <input type="file" className="hidden" id={id+inputIdx} onChange={(e) => handleChange(e,inputIdx)} accept="image/png, image/jpeg, image/jpg"/>
              <div className="flex gap-4">
                <button onClick={() => document.getElementById(id+inputIdx).click()} className="border border-[#D0D5DD] bg-[#F2F4F7] py-[6px] px-4 rounded-lg"><span>Upload File</span></button>
                {
                ...
                <button className={`self center bg-red-600 rounded-lg py-2 px-6 text-white ${inputsFoto.length > 1 ? "" : "hidden"}`} onClick={() => deleteInputFoto(inputIdx)}>Hapus</button>
              </div>
            </div>
          )
        })
      ...
      <div className="pt-4 flex gap-3">
        <button className="bg-green-800 p-2 text-white rounded-lg" onClick={() => tambahInputFoto()}>Tambah Input File</button>
        <button className="bg-gray-800 p-2 text-white rounded-lg" onClick={() => ResetInputs()}>Reset</button>
      </div>
    </div>
  )
}
