    ...
    <div className="flex justify-center mx-auto w-[80%]">
      {
        ...
        <div className="w-full">
          <div className="flex justify-between pb-4 pt-10">
            <span onClick={() => {navigate("/")}} className="self-center font-semibold text-[20px] text-[#0A6C62] cursor-pointer"><ArrowBackRoundedIcon/>Kembali</span>
          ...
          <button onClick={() => setShown(!shown)} className="py-3 px-5 my-5 bg-[#0B3B36] text-white rounded-lg ">Tambah Ulasan</button>
          <div className="border-b border-4 rounded-xl border-[#0B3B36] bg-[#0B3B36]"></div>
          <div className={`my-4  ${shown ? "block" : "hidden"}`}>
            <InputComment value={value} setValue={setValue}/>
            <button onClick={handleSubmitUlasan} className="py-3 px-5 my-5 bg-[#0B3B36] text-white rounded-lg">Submit</button>
          </div>
          <div className="">
            {
              dataUlasan.length < 1 ? 
              ...
              :
              dataUlasan.map((ulasan, ulasanIdx) => (
                <CommentBox key={ulasanIdx} text={ulasan.text} profile_pic={ulasan.profile} img={ulasan.img} nama={ulasan.nama} date={ulasan.date}/>
              ))
            }
          </div>
        </div>
