    ...
    <>
    {
      ...
      <div className="bg-white shadow-lg p-8 mb-8">
        ...
        <div className="border-b-[3px] flex gap-2 text-[#252A31] mb-6">
          <button className={`border-b-[3px] py-2 px-4 translate-y-[3px] ${active === 0 ? "border-[#0B3B36]" : ""}`} onClick={() => {setActive(0)}}>Museum</button>
          <button className={`border-b-[3px] py-2 px-4 translate-y-[3px] ${active === 1 ? "border-[#0B3B36]" : ""}`} onClick={() => {setActive(1)}}>Foto</button>
        </div>
          ...
          <div>
            {
              ...
              <ul className="list-disc">
              {
                errorMessage.map((message, index) => (
                  <li key={index} className="text-sm text-red-600 font-normal leading-5 tracking-tight">{message}</li>
                ))
              }
              </ul>
              : ""
          ...
          <div className="flex justify-end py-2">
            {
              ...
              :
              <button className="py-[10px] px-8 text-white bg-[#0B3B36] text-sm leading-5 tracking-[0.25px] font-medium rounded-lg" onClick={() => HandleSubmit()}>Simpan Dan Unggah</button>
            }
          </div>
        </div>
